...
#
#     # 确保目录存在
#     output_dir = f'Outliers_Removed0/{folder}'
#     if not os.path.exists(output_dir):
#         os.makedirs(output_dir)
#
#     # 保存处理后的网格
...
tolerance = 300  # 容忍范围，接近目标顶点数时停止迭代
            ...
            if file.endswith('.obj'): #filter .obj
                ...
                try:
                    ...
                    while abs(len(mesh_o3d.vertices) - target_vertex_count) > tolerance:
                        ...
                        if abs(current_vertex_count - target_vertex_count) > tolerance:
                            ...
                            if len(mesh_o3d.vertices) == current_vertex_count or iterations == max_iterations:  # Prevent infinite loops
                                print(f"Resampling no longer better for {file}.")
                                break
                        else:
                            break

                    # Save the final processed mesh
