                ...
                try:
                    ...
                    if abs(original_vertex_count - target_vertex_count) <= tolerance:
                        ...
                        ms.meshing_isotropic_explicit_remeshing(
                            targetlen=pml.PercentageValue(), iterations=20)

                        while abs(
                                ms.current_mesh().vertex_number() - target_vertex_count) > tolerance:
                            ...
                            if new_vertex_count == target_vertex_count:
                                break

                        # 保存处理后的网格
                        ms.save_current_mesh(output_path)
                        if iterations == 0:
                            print(f"Final mesh saved as {output_path} (final vertices: {current_vertex_count})")
                        else:
